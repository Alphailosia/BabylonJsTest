...
function startGame() {
    ...
    engine.runRenderLoop(() => {
        ...
        tank.move();
        for(let ntm=0;ntm<tirs.length;ntm++){
            tirs[ntm].move();
        }
...
function createGround(scene) {
    ...
    function onGroundCreated() {
        ...
        groundMaterial.diffuseTexture = new BABYLON.Texture("images/lave.jpg");
...
function tir(scene){
    let sphere = BABYLON.MeshBuilder.CreateSphere("balle", {diameter : 0.5, segment:32},scene);
    sphere.position.y=1.1;
    sphere.position.x = tank.position.x;
    sphere.position.z = tank.position.z;

    sphere.frontVector = tank.frontVector;

    sphere.move = () =>{
        sphere.moveWithCollisions(sphere.frontVector.multiplyByFloats(2,2,2));
    }
    return sphere;
}
